...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <div
      className={`fixed top-0 left-0 h-screen w-[17vw] bg-white text-black transition-transform duration-300 transform ${
          ...
          {menuItem.map((item, index) => (
            <li key={index}>
              <NavLink
                to={item.path}
              >
                <div className='pl-[50px] flex hover:bg-gray-200 hover:border-r-4 hover:border-button hover:bg-button hover:bg-opacity-10 hover:text-button transition duration-300 ease-in-out py-6 items-center text-xl font-semibold'>
                  <p>{item.icon}</p>
                  <p>{item.name}</p>
                </div>
              </NavLink>
